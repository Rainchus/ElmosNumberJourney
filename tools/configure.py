...
header = (
    ...
    'CC_HOST = gcc\n'
    'CPP = cpp -P\n'
    'ROM = $BUILD_PATH/elmosNumberJourney.z64\n'
    'ELF = $BUILD_PATH/elmosNumberJourney.elf\n'
    "LD_SCRIPT = elmosnumberjourney.ld\n"
    'LD_MAP = $BUILD_PATH/elmosNumberJourney.map\n'
...
ninja_file.rule('make_rom_z64',
                command = "(cp $in $out) && (sha1sum -c elmosNumberJourney.sha1) && (python3 ./$MAKE_EXPECTED $out)",
...
ninja_file.build("build/elmosNumberJourney.elf", "make_elf ", o_files)
ninja_file.build("build/elmosNumberJourney.bin", "make_rom_bin ", "build/elmosNumberJourney.elf")
ninja_file.build("build/elmosNumberJourney.z64", "make_rom_z64 ", "build/elmosNumberJourney.bin")
